...
class GenericEvent():
    ...
    def __eq__(self, other):
        ...
        if self.type == 'timeSignature':
            if self.numer != other.numer or \
                self.denom != other.denom or \
                self.metro != other.metro or \
                self.thirtySecondNotes != other.thirtySecondNotes:
                return False
...
class MIDITrack:
    ...
    class timeSignature(GenericEvent):
        '''A class that encapsulates a tempo meta-event
        '''
        def __init__(self,time,numer,denom,metro,thirtySecondNotes):
    ...
    def addTimeSignature(self,time,numer,denom,metro,thirtySecondNotes):
        '''
        Add a tempo change (or set) event.
        '''
        self.eventList.append(MIDITrack.timeSignature(time,numer,denom,metro,thirtySecondNotes))
    ...
    def processEventList(self):
        ...
        for thing in self.eventList:
            if thing.type == 'note':
                ...
                event = MIDIEvent()
                event.type = "NoteOff"
                event.time = (thing.time * TICKSPERBEAT + thing.duration * TICKSPERBEAT)
                event.pitch = thing.pitch
                event.volume = thing.volume
                event.channel = thing.channel
                event.ord = 2
                ...
                event.time = thing.time * TICKSPERBEAT
                event.numer = thing.numer
                event.denom = thing.denom
                event.metro = thing.metro
                event.thirtySecondNotes = thing.thirtySecondNotes
                event.ord = 4
                self.MIDIEventList.append(event)
    ...
    def writeEventsToStream(self):
        ...
        for event in self.MIDIEventList:
            if event.type == "NoteOn":
                ...
                self.MIDIdata = self.MIDIdata + struct.pack('>B', event.numer)
                self.MIDIdata = self.MIDIdata + struct.pack('>B', event.denom)
                self.MIDIdata = self.MIDIdata + struct.pack('>B', event.metro)
                self.MIDIdata = self.MIDIdata + struct.pack('>B', event.thirtySecondNotes)
...
class MIDIFile:
    ...
    def addTimeSignature(self,track, time, numer, denom, metro, thirtySecondNotes):
        """
        Add a tempo event.
        ...
        Arguments:
            ...
            tempo: The tempo, in Beats per Minute. [Integer]
        """
        self.tracks[track].addTimeSignature(time, numer, denom, metro, thirtySecondNotes)
